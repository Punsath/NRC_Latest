...
export const PackagesAdd = () => {
  ...
  const validate = (data) => {
    ...
    if (!data.Title) {
      errors.Title = "Travel Package Name is required.";
    }
  ...
  const handleSubmit = async (event) => {
    ...
    if (Object.keys(validateErrors).length === 0 && Picture) {
      const result = await Swal.fire({
        text: "Are you sure you want to add the package?",
      ...
      if (result.isConfirmed) {
        ...
        try {
          await axiosClient.post(`/packeges`, formDataToSend, {
            headers: {
